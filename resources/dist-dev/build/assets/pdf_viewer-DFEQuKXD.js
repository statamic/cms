...
var { AbortException, AnnotationEditorLayer, AnnotationEditorParamsType, AnnotationEditorType, AnnotationEditorUIManager, AnnotationLayer, AnnotationMode, AnnotationType, applyOpacity, build, ColorPicker, createValidAbsoluteUrl, CSSConstants, DOMSVGFactory, DrawLayer, FeatureTest, fetchData, findContrastColor, getDocument, getFilenameFromUrl, getPdfFilenameFromUrl, getRGB, getRGBA, getUuid, GlobalWorkerOptions, ImageKind, InvalidPDFException, isDataScheme, isPdfFile, isValidExplicitDest, makeArr, makeMap, makeObj, makeSet, MathClamp, noContextMenu, normalizeUnicode, OPS, OutputScale, PasswordException, PasswordResponses, PDFDataRangeTransport, PDFDateString, PDFWorker, PermissionFlag, PixelsPerInch, RenderingCancelledException, renderRichText, ResponseException, setLayerDimensions, shadow, SignatureExtractor, stopEvent, SupportedImageMimeTypes, TextLayer, TextLayerImages, TouchManager, updateUrlHash, Util, VerbosityLevel, version, XfaLayer } = globalThis.pdfjsLib;
...
function isWordBreakAt(content, pos) {
	...
	if (after.index !== pos) return false;
	...
	if (!NormalizeWithNFKC) {
		...
		for (let i = 0; i < 65536; i++) {
			...
			if (c.normalize("NFKC") !== c && !diacriticsRegex.test(c)) {
				...
				if (range[1] + 1 !== i) {
					if (range[0] === range[1]) ranges.push(String.fromCharCode(range[0]));
					else ranges.push(`${String.fromCharCode(range[0])}-${String.fromCharCode(range[1])}`);
					range[0] = range[1] = i;
				} else range[1] = i;
			}
		}
		const rangesStr = ranges.join("");
		if (!NormalizeWithNFKC) NormalizeWithNFKC = rangesStr;
		else if (rangesStr !== NormalizeWithNFKC) {
			for (let i = 1; i < rangesStr.length; i++) if (rangesStr[i] !== NormalizeWithNFKC[i]) {
...
var internalOpt = Object.freeze({ internal: INTERNAL_EVT });
var FindState = {
	FOUND: 0,
	NOT_FOUND: 1,
	WRAPPED: 2,
...
var CHARACTERS_TO_NORMALIZE = new Map([
	["‐", "-"],
	["‘", "'"],
	["’", "'"],
	["‚", "'"],
...
var PDFFindController = class {
	...
	#convertToRegExpString(query, hasDiacritics) {
		...
		query = query.replaceAll(SPECIAL_CHARS_REG_EXP, (match, p1, p2, p3, p4, p5) => {
			if (p1) return addExtraWhitespaces(p1, RegExp.escape(p1));
			if (p2) return addExtraWhitespaces(p2, RegExp.escape(p2));
			if (p3) return "[ ]+";
	...
	match(query, pageContent, pageIndex) {
		...
		while ((match = query.exec(pageContent)) !== null) {
			if (entireWord && !isEntireWord(pageContent, match.index, match[0].length)) continue;
			matches.push({
				index: match.index,
	...
	#extractText() {
		...
		for (let i = 0, ii = this._linkService.pagesCount; i < ii; i++) {
			...
			deferred = deferred.then(async () => {
				...
				try {
					const textContent = await (await pdfDoc.getPage(i + 1)).getTextContent(textOptions);
					if (pdfDoc !== this._pdfDocument) {
						resolve();
						return;
					}
	...
	#onPagesEdited({ pagesMapper, type, pageNumbers }) {
		...
		if (type === "copy") {
			...
			const diacritics = /* @__PURE__ */ new Map();
		...
		if (type === "delete") this.#savedPageData = {
...
var PDFLinkService = class {
	...
	async getAttachmentContent(id) {
		try {
			...
			if (!(error instanceof PasswordException)) console.warn(`Unable to load attachment content: ${error}`);
		}
		return null;
	}
...
var AnnotationLayerBuilder = class {
	...
	#updatePresentationModeState(state) {
		...
		for (const section of this.div.childNodes) {
			if (section.hasAttribute("data-internal-link") || section.classList.contains("mediaAnnotation")) continue;
...
var BaseDownloadManager = class {
	...
	openOrDownloadData(data, filename, dest = null) {
		...
		if (isPdfData) {
			const blobUrl = this.#openBlobUrls.getOrInsertComputed(data, () => URL.createObjectURL(new Blob([data], { type: contentType })));
			try {
	...
	download(data, url, filename) {
		const blobUrl = data ? URL.createObjectURL(new Blob([data], { type: "application/pdf" })) : null;
		this._triggerDownload(blobUrl, url, filename);
	}
};
var DownloadManager = class extends BaseDownloadManager {
	_triggerDownload(blobUrl, originalUrl, filename, isAttachment = false) {
		if (!blobUrl && !isAttachment) {
			if (!createValidAbsoluteUrl(originalUrl, "http://example.com")) throw new Error(`_triggerDownload - not a valid URL: ${originalUrl}`);
...
var EventBus = class {
	...
	dispatch(eventName, data) {
		...
		if (extListeners) for (const listener of extListeners) listener(data);
	}
};
...
};
{
	...
	if (/\b(?:iPad|iPhone|iPod)(?=;)/.test(userAgent) || platform === "MacIntel" && maxTouchPoints > 1 || isAndroid) compatParams.set("maxCanvasPixels", 5242880);
...
var AppOptions = class {
	...
	static {
		for (const [name, defaultOpt] of defaultOptions) this.#opts.set(name, defaultOpt.value);
	...
	static setAll(options, prefs = false) {
		...
		for (const name in options) {
			const defaultOpt = defaultOptions.get(name), userOpt = options[name];
...
var Autolinker = class {
	...
	static findLinks(text) {
		this.#regex ??= /* @__PURE__ */ new RegExp("\\b(?:https?:\\/\\/|mailto:|www\\.)(?:[\\S--[\\p{P}<>]]|\\/|[\\S--[\\[\\]]]+[\\S--[\\p{P}<>]])+|(?=\\p{L})[\\S--[@\\p{Ps}\\p{Pe}<>]]+@([\\S--[[\\p{P}--\\-]<>]]+(?:\\.[\\S--[[\\p{P}--\\-]<>]]+)+)", "gv");
		...
		for (const match of matches) {
			...
			if (url.startsWith("www.") || url.startsWith("http://") || url.startsWith("https://")) raw = url;
				...
				if (this.#numericTLDRegex.test(hostname)) continue;
			}
...
var BasePDFPageView = class extends RenderableView {
	#loadingId = null;
	#renderError = null;
	...
	async _drawCanvas(options, onCancel, onFinish) {
		...
		try {
			...
			if (renderTask === this.renderTask) {
				...
				if (this.imagesRightClickMinSize !== -1) this.imageCoordinates ??= this.pdfPage.imageCoordinates;
			}
		}
...
var StructTreeLayerBuilder = class {
	...
	#collectIds(node, ids) {
		...
		for (const kid of node.children || []) this.#collectIds(kid, ids);
	}
	#walk(node, parentNodes = []) {
		...
		if ("role" in node) {
			...
			if (MathMLElements.has(role)) {
				...
				for (const { type, id } of node.children || []) if (type === "content" && id) ids.push(id);
			} else element = document.createElement("span");
			const match = role.match(HEADING_PATTERN);
			if (match) {
				element.setAttribute("role", "heading");
				element.setAttribute("aria-level", match[1]);
			} else if (PDF_ROLE_TO_HTML_ROLE[role]) element.setAttribute("role", role === "TH" && parentNodes.at(-1)?.role === "TR" && parentNodes.at(-2)?.role === "TBody" ? "rowheader" : PDF_ROLE_TO_HTML_ROLE[role]);
			if (role === "Figure" && this.#addImageInTextLayer(node, element)) return element;
			if (role === "Formula") {
				if (node.mathML && MathMLSanitizer.sanitizer) {
					visitChildren = false;
					element.setHTML(node.mathML, { sanitizer: MathMLSanitizer.sanitizer });
		...
		if (node.children) {
			if (node.children.length === 1 && "id" in node.children[0]) this.#setAttributes(node.children[0], element);
				...
				for (const kid of node.children) element.append(this.#walk(kid, parentNodes));
				parentNodes.pop();
			}
		}
		return element;
	...
	_renderMatches(matches) {
		...
		function appendTextToDiv(divIdx, fromOffset, toOffset, className) {
			...
			if (className) {
				...
				span.append(node);
				div.append(span);
				if (className.includes("selected")) return span;
				return null;
			}
			div.append(node);
			return 0;
		...
		for (let i = i0; i < i1; i++) {
			...
			if (begin.divIdx === end.divIdx) selectedSpan = appendTextToDiv(begin.divIdx, begin.offset, end.offset, "highlight" + highlightSuffix);
			else {
...
var TextLayerBuilder = class TextLayerBuilder {
	...
	static #enableGlobalSelectionListener(globalAbortSignal) {
		if (this.#selectionChangeAC) return;
		...
		document.addEventListener("selectionchange", () => {
			...
			if (isFirefoxOrModernChromium === void 0) {
				...
				if (!isFirefoxOrModernChromium) {
					const chromiumVersion = navigator.userAgentData ? navigator.userAgentData.brands.find(({ brand }) => brand === "Chromium")?.version : /\bChrome\/(\d+)\b/.exec(navigator.userAgent)?.[1];
...
var XfaLayerBuilder = class {
	...
	async render({ viewport, intent = "display" }) {
		let xfaHtml;
		if (intent === "print") xfaHtml = this.xfaHtml;
		else {
...
var PDFPageView = class PDFPageView extends BasePDFPageView {
	#abortSignal = null;
	...
	updatePageNumber(newPageNumber) {
		...
		if (oldPageNumber !== newPageNumber) {
			...
			if (this.pdfPage) this.pdfPage.pageNumber = newPageNumber;
	...
	async #renderAnnotationLayer(textLayerPromise = null) {
		...
		if (this.#enableAutoLinking && textLayerPromise) this.#injectLinkAnnotations(textLayerPromise, annotationLayer, textLayer);
	}
	_refreshAnnotationLayer() {
		if (this._annotationCanvasMap?.size) this.annotationLayer?.refreshCanvases();
	}
	...
	async #injectLinkAnnotations(textLayerPromise, annotationLayer, textLayer) {
		...
		try {
			...
			if (annotationLayer !== this.annotationLayer || textLayer !== this.textLayer) return;
	...
	async draw() {
		...
		if (!this.textLayer && this.#textLayerMode !== TextLayerMode.DISABLE && !pdfPage.isPureXfa) {
			...
			if (this.enableSelectionRendering) this.textLayer.div.classList.add("selectionRendering");
		}
		...
		const resultPromise = this._drawCanvas(this._getRenderingContext(canvas, transform, recordBBoxes, recordImages), () => {
			...
			if (this.annotationLayer) await this.#renderAnnotationLayer(textLayerPromise);
...
var PDFScriptingManager = class {
	...
	async setDocument(pdfDocument) {
		...
		if (objects) {
			...
			for (const fields of Object.values(objects)) for (const { id } of fields) this.#objectIds.add(id);
		}
		try {
	...
	async #updateFromSandbox(detail) {
		...
		for (const elementId of ids) {
			if (!this.#objectIds?.has(elementId)) continue;
...
var PDFScriptingManagerComponents = class extends PDFScriptingManager {
	constructor(options) {
		...
		options.externalServices ||= { createScripting: () => new GenericScripting(options.sandboxBundleSrc, options.wasmUrl) };
...
var PDFViewer = class {
	...
	constructor(options) {
		...
		this.eventBus.on("thumbnailrendered", ({ pageNumber, pdfPage }) => {
	...
	clearSelection() {
		...
		for (let i = 0, ii = selection.rangeCount; i < ii; i++) if (selection.getRangeAt(i).intersectsNode(this.viewer)) {
	...
	set currentPageNumber(val) {
		...
		if (!this.pdfDocument) return;
		if (this._currentPageNumber !== val) this.clearSelection();
	...
	set currentPageLabel(val) {
		...
		if (this._currentPageNumber !== page) this.clearSelection();
		if (!this._setCurrentPageNumber(page, true)) console.error(`currentPageLabel: "${val}" is not a valid page.`);
	}
	get currentScale() {
		return this._currentScale !== 0 ? this._currentScale : 1;
	}
	...
	async getAllText(interruptSignal = null) {
		...
		for (let pageNum = 1, pagesCount = this.pdfDocument.numPages; pageNum <= pagesCount; ++pageNum) {
			if (interruptSignal?.aborted) return null;
			buffer.length = 0;
	...
	#copyCallback(textLayerMode, event) {
		...
		if (anchorNode && focusNode && selection.containsNode(this.#hiddenCopyElement)) {
			...
			const keydownAC = new AbortController(), interruptAC = new AbortController();
			window.addEventListener("keydown", (ev) => {
				if (ev.key === "Escape") interruptAC.abort();
	...
	onPagesEdited({ pagesMapper, type, hasBeenCut, pageNumbers }) {
		if (type === "copy") {
			this.#copiedPageViews = /* @__PURE__ */ new Map();
			for (const pageNum of pageNumbers) this.#copiedPageViews.set(pageNum, this._pages[pageNum - 1]);
			return;
		}
		if (type === "cancelCopy") {
		...
		if (type === "cleanSavedData") {
			if (this.#deletedPageNumbers) {
				if (this.#savedPageViews) {
					for (const pageNum of this.#deletedPageNumbers) this.#savedPageViews[pageNum - 1].deleteMe();
					this.#savedPageViews = null;
				}
				this.#deletedPageNumbers = null;
			}
		...
		for (let i = 1, ii = pagesMapper.pagesNumber; i <= ii; i++) {
			...
			if (prevPageNumber < 0) {
				...
				if (hasBeenCut) page.updatePageNumber(i);
				else {
		...
		if (viewerElement) {
			const fragment = document.createDocumentFragment();
			for (const { div } of newPages) fragment.append(div);
			viewerElement.replaceChildren(fragment);
		}
		setTimeout(() => {
			this.forceRendering();
		...
		if (scale > 0) {
			...
			switch (value) {
				...
				case "auto":
					...
					scale = Math.min(1.25, horizontalScale);
					break;
				default:
	...
	scrollPageIntoView({ pageNumber, destArray = null, allowNegativeOffset = false, ignoreDestinationZoom = false, center = null }) {
		...
		if (!ignoreDestinationZoom) {
			...
			else if (this._currentScale === 0) this.currentScaleValue = "auto";
		}
	...
	updateScale({ drawingDelay, scaleFactor = null, steps = null, origin }) {
		...
		newScale = MathClamp(newScale, .1, 25);
...
//#endregion
export { AnnotationLayerBuilder, EventBus, PDFLinkService };
