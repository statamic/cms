    ...
    @else
            ...
            <table class="data-table">
                <tr>
                    <td class="w-64 font-bold">
                        <span class="little-dot {{ $site->valid() ? 'bg-green-600' : 'bg-rose-500' }} mr-2"></span>
                    ...
                    <td class="text-right text-rose-500">{{ $site->invalidReason() }}</td>
                </tr>
            </table>
        </div>
        ...
        <div class="card p-0 mt-2">
            <table class="data-table">
                <tr>
                    <td class="w-64 font-bold">
                        <span class="little-dot {{ $statamic->valid() ? 'bg-green-600' : 'bg-rose-500' }} mr-2"></span>
                        Statamic @if ($statamic->pro())<span class="text-pink">Pro</span>@else Free @endif
                    </td>
                    <td>{{ $statamic->version() }}</td>
                    <td class="text-right text-rose-500">{{ $statamic->invalidReason() }}</td>
                </tr>
            </table>
        </div>
        ...
        <div class="card p-0 mt-2">
            <table class="data-table">
                @foreach ($addons as $addon)
                    <tr>
                        <td class="w-64 mr-2">
                            <span class="little-dot {{ $addon->valid() ? 'bg-green-600' : 'bg-rose-500' }} mr-2"></span>
                        ...
                        <td>{{ $addon->version() }}</td>
                        <td class="text-right text-rose-500">{{ $addon->invalidReason() }}</td>
                    </tr>
                @endforeach
            </table>
        </div>
