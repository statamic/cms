...
@section('content')
    ...
    <div class="flex justify-between items-center mb-6">
        ...
        <dropdown-list class="inline-block">
            <template v-slot:trigger>
                <button class="button btn-primary flex items-center pe-4">
                    {{ __('Create Blueprint') }}
    ...
    @foreach (Statamic\Facades\Collection::all() as $collection)
        @if ($loop->first)
        <h3 class="little-heading ps-0 mb-2">{{ __('Collections') }}</h3>
                    ...
                    <tr>
                        <td>
                            <div class="flex items-center">
                                <div class="w-4 h-4 me-4">@cp_svg('icons/light/content-writing')</div>
                                <span class="little-dot {{ $blueprint->hidden() ? 'hollow' : 'bg-green-600' }} me-2" v-tooltip="'{{ __($blueprint->hidden() ? 'Hidden': 'Visible') }}'"></span>
                                <a href="{{ cp_route('collections.blueprints.edit', [$collection, $blueprint]) }}" v-pre>{{ __($blueprint->title()) }}</a>
                            </div>
                        </td>
                        <td class="text-end text-2xs" v-pre>{{ __($collection->title()) }}</td>
                    </tr>
                @endforeach
        @if ($loop->last)
            </table>
    ...
    @foreach (Statamic\Facades\Taxonomy::all() as $taxonomy)
        @if ($loop->first)
        <h3 class="little-heading ps-0 mb-2">{{ __('Taxonomies') }}</h3>
        <div class="card p-0 mb-4">
            <table class="data-table">
        @endif
                @foreach ($taxonomy->termBlueprints() as $blueprint)
                    <tr>
                        <td>
                            <div class="flex items-center">
                                <div class="w-4 h-4 me-4">@cp_svg('icons/light/tags')</div>
                                <span class="little-dot {{ $blueprint->hidden() ? 'hollow' : 'bg-green-600' }} me-2" v-tooltip="'{{ __($blueprint->hidden() ? 'Hidden': 'Visible') }}'"></span>
                                <a href="{{ cp_route('taxonomies.blueprints.edit', [$taxonomy, $blueprint]) }}" v-pre>{{ __($blueprint->title()) }}</a>
                            </div>
                        </td>
                        <td class="text-end text-2xs" v-pre>{{ __($taxonomy->title()) }}</td>
                    </tr>
                @endforeach
        @if ($loop->last)
            </table>
    ...
    @foreach (Statamic\Facades\Nav::all() as $nav)
        @if ($loop->first)
        <h3 class="little-heading ps-0 mb-2">{{ __('Navigation') }}</h3>
        <div class="card p-0 mb-4">
            <table class="data-table">
        @endif
                <tr>
                    <td>
                        <div class="flex items-center">
                            <div class="w-4 h-4 me-4">@cp_svg('icons/light/hierarchy-files')</div>
    ...
    @foreach (Statamic\Facades\GlobalSet::all() as $set)
        @if ($loop->first)
        <h3 class="little-heading ps-0 mb-2">{{ __('Globals') }}</h3>
        <div class="card p-0 mb-4">
            <table class="data-table">
        @endif
                <tr>
                    <td>
                        <div class="flex items-center">
                            <div class="w-4 h-4 me-4">@cp_svg('icons/light/earth')</div>
    ...
    @foreach (Statamic\Facades\AssetContainer::all() as $container)
        @if ($loop->first)
        <h3 class="little-heading ps-0 mb-2">{{ __('Asset Containers') }}</h3>
        <div class="card p-0 mb-4">
            <table class="data-table">
        @endif
                <tr>
                    <td>
                        <div class="flex items-center">
                            <div class="w-4 h-4 me-4">@cp_svg('icons/light/assets')</div>
    ...
    @if (Statamic\Facades\User::current()->can('configure form fields'))
        @foreach (Statamic\Facades\Form::all() as $form)
            @if ($loop->first)
        <h3 class="little-heading ps-0 mb-2">{{ __('Forms') }}</h3>
        <div class="card p-0 mb-2">
            ...
            @endif
                <tr>
                    <td>
                        <div class="flex items-center">
                            <div class="w-4 h-4 me-4">@cp_svg('icons/light/drawer-file')</div>
    ...
    <h3 class="little-heading ps-0 mb-2">{{ __('Users') }}</h3>
    <div class="card p-0 mb-4">
        <table class="data-table">
            <tr>
                <td>
                    <div class="flex items-center">
                        <div class="w-4 h-4 me-4">@cp_svg('icons/light/users')</div>
            ...
            <tr>
                <td>
                    <div class="flex items-center">
                        <div class="w-4 h-4 me-4">@cp_svg('icons/light/user_groups')</div>
    ...
    <h3 class="little-heading ps-0 mb-2">{{ $namespace['title'] }}</h3>
    <div class="card p-0 mb-4">
        <table class="data-table">
        @foreach ($namespace['blueprints'] as $blueprint)
            <tr>
                <td>
                    <div class="flex items-center">
                        <div class="w-4 h-4 me-4">@cp_svg('icons/light/blueprint')</div>
