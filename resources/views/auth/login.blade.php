...
@section('content')
    <div class="relative mx-auto max-w-[400px] items-center justify-center">
        ...
        >
            <login :has-error="{{ $str::bool(count($errors) > 0) }}" v-slot="{ busy, setBusy, hasError }">
                <div>
                    @if ($emailLoginEnabled)
                        <form method="POST" class="flex flex-col gap-6" @submit="setBusy(true)">
                            ...
                            <ui-field :label="__('Email')" error="{{ $errors->first('email') }}">
                                <ui-input name="email" value="{{ old('email') }}" autofocus tabindex="1" />
                            </ui-field>

                            <ui-field :label="__('Password')" error="{{ $errors->first('password') }}">
                                <ui-input name="password" type="password" value="{{ old('password') }}" tabindex="2" />
                                <template #actions>
                                    <a
