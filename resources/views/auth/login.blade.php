...
@section('content')
    ...
    <div class="max-w-xs rounded shadow-lg flex items-center justify-center relative mx-auto">
        ...
        <div class="card auth-card">
            <login :show-email-login="!{{ $str::bool($oauth) }}" :has-error="{{ $str::bool(count($errors) > 0) }}">
                <template #default="{ showEmailLogin, hasError }">
                    <div>
                        @if ($oauth)
                        ...
                        <form method="POST" v-show="showEmailLogin" class="email-login select-none"
                            @if ($oauth) v-cloak @endif @submit="busy = true">
                            {!! csrf_field() !!}
                            ...
                            <div class="mb-8">
                                ...
                                <input type="text" class="input-text input-text" name="email" value="{{ old('email') }}"
                                       autofocus id="input-email">
                                @if ($hasError('email'))
                            ...
                            <div class="mb-8">
                                ...
                                <input type="password" class="input-text input-text" name="password"
                                       id="input-password">
