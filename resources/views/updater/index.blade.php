            ...
            <table class="data-table">
                <tr>
                    ...
                    @if ($count = $statamic->availableUpdatesCount())
                        <td class="text-end"><span class="badge-sm bg-green-600 btn-xs">{{ trans_choice('1 update|:count updates', $count) }}</span></td>
                    @else
                        <td class="text-end">{{ __('Up to date') }}</td>
                    @endif
                </tr>
            </table>
        </div>
            ...
            <table class="data-table">
                ...
                <tr>
                    <td class="w-64"><a href="{{ route('statamic.cp.updater.product', $addon -> slug()) }}"
                            class="text-blue font-bold me-2">{{ $addon -> name() }}</a>
                    <td>{{ $addon -> version() }}</td>
