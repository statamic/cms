...
<div class="card">
    <form method="POST" action="{{ cp_route('utilities.email') }}">
        ...
        <div class="flex items-center">
            <input
                class="input-text ltr:mr-4 rtl:ml-4"
                type="text"
                name="email"
...
<h2 class="mb-2 mt-10 text-lg font-bold">{{ __('Configuration') }}</h2>
...
<div class="card p-0">
    <table class="data-table">
        <tr>
            <th class="w-1/4 py-2 ltr:pl-4 rtl:pr-4">{{ __('Default Mailer') }}</th>
            <td><code>{{ config('mail.default') }}</code></td>
        </tr>
        @if (config('mail.default') == 'smtp')
            <tr>
                <th class="w-1/4 py-2 ltr:pl-4 rtl:pr-4">{{ __('Host') }}</th>
                <td><code>{{ config('mail.mailers.smtp.host') }}</code></td>
            </tr>
            <tr>
                <th class="w-1/4 py-2 ltr:pl-4 rtl:pr-4">{{ __('Port') }}</th>
                <td><code>{{ config('mail.mailers.smtp.port') }}</code></td>
            </tr>
            <tr>
                <th class="w-1/4 py-2 ltr:pl-4 rtl:pr-4">{{ __('Encryption') }}</th>
                <td>
            ...
            <tr>
                <th class="w-1/4 py-2 ltr:pl-4 rtl:pr-4">{{ __('Username') }}</th>
                <td>
            ...
            <tr>
                <th class="w-1/4 py-2 ltr:pl-4 rtl:pr-4">{{ __('Password') }}</th>
                <td>
        ...
        @if (config('mail.default') == 'sendmail')
            <tr>
                <th class="w-1/4 py-2 ltr:pl-4 rtl:pr-4">{{ __('Sendmail') }}</th>
                <td><code>{{ config('mail.mailers.sendmail.path') }}</code></td>
            </tr>
        @endif

        <tr>
            <th class="w-1/4 py-2 ltr:pl-4 rtl:pr-4">{{ __('Default From Address') }}</th>
            <td>
        ...
        <tr>
            <th class="w-1/4 py-2 ltr:pl-4 rtl:pr-4">{{ __('Default From Name') }}</th>
            <td>
        ...
        <tr>
            <th class="w-1/4 py-2 ltr:pl-4 rtl:pr-4">{{ __('Markdown theme') }}</th>
            <td>
        ...
        <tr>
            <th class="w-1/4 py-2 ltr:pl-4 rtl:pr-4">{{ __('Markdown paths') }}</th>
            <td>
