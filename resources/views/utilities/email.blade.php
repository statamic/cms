...
@section('content')
    ...
    <div class="card">
        <form method="POST" action="{{ cp_route('utilities.email') }}">
            ...
            @if ($errors->has('email'))
                <p class="mt-2___REPLACED"><small class="help-block text-red">{{ $errors->first('email') }}</small></p>
            @endif
        </form>
    </div>

    <h2 class="mt-10___REPLACED mb-2___REPLACED font-bold text-lg">{{ __('Configuration') }}</h2>
    <p class="text-sm text-grey mb-4___REPLACED">{!! __('statamic::messages.email_utility_configuration_description', ['path' => config_path('mail.php')]) !!}</p>
    <div class="card p-0">
