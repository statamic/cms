...
@section('content')
    ...
    <div class="card p-0">
        @if ($errors->has('indexes'))
            <p class="p-4___REPLACED"><small class="help-block text-red">{{ $errors->first() }}</small></p>
        @endif
                ...
                @foreach (\Statamic\Facades\Search::indexes() as $index)
                    <tr>
                        <td class="flex items-center">
                            @cp_svg('search-drivers/' . $index->config()['driver'], 'w-6 h-6 mr-2___REPLACED')
                            ...
                            @else
                                <div class="text-sm text-grey flex flex-wrap">
                                    @foreach($index->config()['searchables'] as $searchable)
                                        <div class="mr-2___REPLACED badge-pill-sm">
                                            {{ $searchable }}
                                        </div>
                                    @endforeach
                                </div>
