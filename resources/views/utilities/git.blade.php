        ...
        @forelse($statuses ?? [] as $path => $status)
            <div class="p-4 {{ $loop->first ? '' : 'border-t dark:border-dark-900' }}">
                ...
                <div class="mt-4 text-sm text-gray dark:text-dark-150 flex">
                    <div class="me-4 badge-pill-sm"><span class="text-gray-800 dark:text-dark-150 font-medium">{{ __('Affected files') }}:</span> {{ $status->totalCount }}</div>
                    @if ($status->addedCount)
                        <div class="me-4 badge-pill-sm"><span class="text-gray-800 dark:text-dark-150 font-medium">{{ __('Added') }}:</span> {{ $status->addedCount }}</div>
                    @endif
                    @if ($status->modifiedCount)
                        <div class="me-4 badge-pill-sm"><span class="text-gray-800 dark:text-dark-150 font-medium">{{ __('Modified') }}:</span> {{ $status->modifiedCount }}</div>
                    @endif
                    @if ($status->deletedCount)
                        <div class="me-4 badge-pill-sm"><span class="text-gray-800 dark:text-dark-150 font-medium">{{ __('Deleted') }}:</span> {{ $status->deletedCount }}</div>
                    @endif
                </div>
            </div>
        @empty
