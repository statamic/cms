@php
    ...
    $optionClass = 'relative flex size-9 shrink-0 cursor-pointer items-center justify-center rounded-md border border-gray-300 shadow-ui-xs bg-white text-xs font-semibold text-gray-800 transition-colors hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200 dark:hover:bg-gray-925';
...
<div class="inline-flex w-fit max-w-full flex-col gap-2" data-opinion-scale id="{{ $id }}">
    ...
    @if (! empty($low_label) || ! empty($middle_label) || ! empty($high_label))
        <div class="grid w-full grid-cols-[1fr_auto_1fr] gap-2 px-0.75 text-xs text-gray-500 dark:text-gray-400">
            @if (! empty($low_label))
