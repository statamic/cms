                ...
                <div>
                    <div class="card overflow-hidden p-0 relative">
                        ...
                        >
                            ...
                            <data-list-search
                                class="h-8 mt-2 min-w-[240px] w-full" ref="search" v-model="searchQuery"
                            ...
                            <div class="flex space-x-2 rtl:space-x-reverse mt-2">
                                ...
                                />
                                <button
                                    class="btn btn-sm rtl:mr-2 ltr:ml-2" v-text="__('Save')" v-show="isDirty"
                        ...
                        <div
                            v-show="items.length === 0" class="p-6 text-center text-gray-500" v-text="__('No results')"
                        />
                        ...
                        <div class="overflow-x-auto overflow-y-hidden">
                            ...
                            >
                                <template #cell-title="{ row: entry }">
                                    <a
                                        class="title-index-field inline-flex items-center" :href="entry.edit_url"
                                        @click.stop
                                    >
                                        <span
                                            class="little-dot rtl:ml-2 ltr:mr-2" v-tooltip="getStatusLabel(entry)"
                                            :class="getStatusClass(entry)" v-if="! columnShowing('status')"
                                        />
                                ...
                                <template #cell-status="{ row: entry }">
                                    <div
                                        class="status-index-field select-none" v-tooltip="getStatusTooltip(entry)"
                                        :class="`status-${entry.status}`" v-text="getStatusLabel(entry)"
                                    />
                                </template>
                                ...
                                <template #actions="{ row: entry, index }">
                                    <dropdown-list placement="left-start">
                                        <dropdown-item
                                            :text="__('View')" :external-link="entry.permalink"
                                            v-if="entry.viewable && entry.permalink"
                                        />
                                        <dropdown-item
                                            :text="__('Edit')" :redirect="entry.edit_url" v-if="entry.editable"
                                        />
