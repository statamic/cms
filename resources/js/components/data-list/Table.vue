    ...
    <table :data-size="relativeColumnsSize" ref="table" tabindex="0" class="data-table" :class="{ 'select-none' : shifting }" @keydown.shift="shiftDown" @keyup="clearShift">
            ...
            <tr>
                ...
                <th
                    ...
                    :class="{
                        ...
                        'text-end pe-8': column.numeric,
                    }"
                    class="group rounded-none text-start"
                    @click.prevent="changeSortColumn(column.field)"
                >
                    <span v-text="__(column.label)" />
                    <svg v-if="column.sortable" :class="[sharedState.sortDirection, {'opacity-100 pointer-events-none': sharedState.sortColumn === column.field}]" height="8" width="8" viewBox="0 0 10 6.5" class="ms-1 opacity-0 group-hover:opacity-100">
        ...
        <tbody>
            ...
            <tr v-for="(row, index) in rows" :key="row.id" class="sortable-row outline-none" :class="{'row-selected': sharedState.selections.includes(row.id)}">
                ...
                <td v-for="column in visibleColumns" :key="column.field" @click="rowClicked(row, index, $event)" :width="column.width" :class="{'text-end pe-8': column.numeric}">
                    <slot
