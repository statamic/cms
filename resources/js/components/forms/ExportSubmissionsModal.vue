...
const hasFilteredScope = computed(() => {
    ...
    const hasSortOverride =
        (params.sort && params.sort !== "datestamp") ||
        (params.order && params.order !== "desc");
    return !!(params.search || params.filters || hasSortOverride);
});

const selectedExporter = computed(() =>
    props.exporters.find((exporter) => exporter.handle === format.value),
);

const canSelectColumns = computed(
    () => selectedExporter.value?.supportsColumnSelection ?? false,
);
...
const canExport = computed(
    () =>
        ...
        (!canSelectColumns.value || selectedColumns.value.length > 0),
);

function toggleAllColumns() {
    selectedColumns.value = allColumnsSelected.value
        ? []
        : props.columns.map((column) => column.handle);
}
    ...
    >
        <div class="space-y-6">
            ...
            <div>
                ...
                <RadioGroup v-model="scope">
                    ...
                    <Radio
                        ...
                        :description="
                            __(
                                'statamic::messages.form_export_filtered_description',
                            )
                        "
            ...
            <div v-if="canSelectColumns">
                ...
                <div
                    class="max-h-48 overflow-y-auto rounded-lg border border-gray-200 dark:border-gray-700 p-3"
                >
