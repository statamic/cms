    ...
    <div>
        <div class="flex items-center mb-6">
            <h1 class="flex-1">
                ...
                <span
                    v-if="currentVersion"
                    class="font-normal text-gray-700 ml-2"
                    >{{ currentVersion }}</span
                >
            </h1>
            <button
                v-if="!onLatestVersion"
        ...
        <div
            class="bg-yellow border-yellow-dark border-dashed p-4 text-xs border mb-6 rounded cursor-pointer flex items-center justify-between"
        ...
        >
            <div>
                <h4
                    v-text="
                        __(
                            'messages.addon_has_more_releases_beyond_license_heading'
                        )
                    "
                />
                <p
                    v-text="
                        __(
                            'messages.addon_has_more_releases_beyond_license_body'
                        )
                    "
                />
            </div>
            <button
                class="btn btn-xs"
                v-text="__('View additional releases')"
            />
        </div>
        ...
        >
            <div class="prose">
                <p
                    v-text="
                        `${__('messages.updater_update_to_latest_command')}:`
                    "
                />
                <code-block copyable :text="command" />
                <p>
                    {{ __("Learn more about") }}
                    <a href="https://statamic.dev/updating" target="_blank">{{
                        __("Updates")
                    }}</a
                    >.
                </p>
...
export default {
    ...
    computed: {
        ...
        onLatestVersion() {
            return (
                this.currentVersion && this.currentVersion == this.latestVersion
            );
        },
