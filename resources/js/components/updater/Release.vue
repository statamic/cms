    ...
    <div class="card update-release mb-10">
        <div class="flex justify-between mb-6">
            <div>
                ...
                <h5
                    class="date"
                    v-text="__('Released on :date', { date: release.date })"
                />
            </div>
    ...
    computed: {
        body() {
            return markdown(this.release.body)
                .replaceAll(
                    "[new]",
                    '<span class="label" style="background: #5bc0de;">NEW</span>'
                )
                .replaceAll(
                    "[fix]",
                    '<span class="label" style="background: #5cb85c;">FIX</span>'
                )
                .replaceAll(
                    "[break]",
                    '<span class="label" style="background: #d9534f;">BREAK</span>'
                )
                .replaceAll(
                    "[na]",
                    '<span class="label" style="background: #e8e8e8;">N/A</span>'
                );
        },
        ...
        installButtonText() {
            ...
            if (this.release.type === "upgrade") {
                return __("Update to :version", {
                    version: this.release.version,
                });
            }

            return __("Downgrade to :version", {
                version: this.release.version,
            });
        },
