<template>
    ...
    >
        <svg-icon v-if="tab.icon" :name="iconName(tab.icon)" :directory="iconBaseDirectory" class="w-4 h-4 me-1" />

        {{ __(tab.display) }}

        <dropdown-list v-if="isActive" ref="dropdown" placement="bottom-start" class="text-start">
            <template #trigger>
                <button class="ms-2 hover:text-gray-900 dark:hover:text-gray-400 active:text-gray-900" :aria-label="__('Open Dropdown')">
