<template>
    ...
    <div class="blueprint-section-field blueprint-section-import blueprint-section-field-w-full">
        <div class="blueprint-section-field-inner">
            <div class="blueprint-drag-handle w-4 border-e dark:border-dark-300"></div>
            <div class="flex flex-1 items-center justify-between">
                <div class="flex items-center flex-1 pe-4 py-2 ps-2">
                    <svg-icon class="flex-none text-gray-700 dark:text-dark-150 h-4 w-4 me-2" name="paperclip" v-tooltip="__('Linked fieldset')" />
                    <a class="break-all" @click="$emit('edit')">
                        <span v-text="__('Fieldset')" />
                        <span class="font-mono text-3xs text-gray-600 dark:text-dark-175 ms-2">{{ field.fieldset }}</span>
                    </a>
                </div>
