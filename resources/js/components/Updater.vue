    ...
    <div>
        <div class="flex mb-3">
            ...
            <button class="btn mr-2" v-if="output" @click="$modal.show('output-modal')">
                <span v-if="output.processing" class="icon icon-circular-graph animation-spin"></span>
                <span v-else>{{ translate('View Output') }}</span>
            </button>
        ...
        <portal to="modals">
            <modal name="output-modal" :scrollable="true" @opened="$events.$emit('start-composer')">
                <composer-output>{{ output.status }}</composer-output>
            </modal>
        </portal>
