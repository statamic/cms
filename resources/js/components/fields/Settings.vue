    ...
    <div class="h-full bg-gray-300 h-full dark:bg-dark-800 overflow-scroll">
        ...
        <header v-if="!loading" class="flex items-center sticky top-0 inset-x-0 bg-white dark:bg-dark-550 shadow dark:shadow-dark px-8 py-2 z-1 h-13">
            <h1 class="flex-1 flex items-center text-xl">
                ...
                <small class="badge-pill bg-gray-100 dark:bg-dark-400 ms-4 border dark:border-dark-200 text-xs text-gray-700 dark:text-dark-150 font-medium leading-none flex items-center">
                    <svg-icon class="h-4 w-4 me-2 inline-block text-gray-700 dark:text-dark-150" :name="fieldtype.icon.startsWith('<svg') ? fieldtype.icon : `light/${fieldtype.icon}`"></svg-icon>
                    {{ fieldtype.title }}
                </small>
            </h1>
            <button
                class="text-gray-700 dark:text-dark-150 hover:text-gray-800 dark:hover:text-dark-100 me-6 text-sm"
                @click.prevent="close"
                v-text="__('Cancel')"
            ></button>
            <button
