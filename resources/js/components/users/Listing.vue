                ...
                <div class="card overflow-hidden p-0 relative">
                    <div class="flex flex-wrap items-center justify-between px-2 pb-2 text-sm border-b dark:border-dark-900">
                        ...
                        <div class="flex space-x-2 rtl:space-x-reverse mt-2">
                            <button class="btn btn-sm ms-2" v-text="__('Reset')" v-show="isDirty" @click="$refs.presets.refreshPreset()" />
                            <button class="btn btn-sm ms-2" v-text="__('Save')" v-show="allowFilterPresets && isDirty" @click="$refs.presets.savePreset()" />
                        ...
                        >
                            <template slot="cell-email" slot-scope="{ row: user, value }">
                                <a :href="user.edit_url" class="flex items-center">
                                    <avatar :user="user" class="w-8 h-8 rounded-full me-2" />
                                    {{ value }}
                                </a>
                            </template>
                            <template slot="cell-roles" slot-scope="{ row: user, value: roles }">
                                <div class="role-index-field">
                                    <div v-if="user.super" class="role-index-field-item me-1 mb-1.5">{{ __('Super Admin') }}</div>
                                    <div v-if="!roles || roles.length === 0" />
                                    <div v-for="(role, i) in (roles || [])" class="role-index-field-item me-1 mb-1.5">{{ __(role.title) }}</div>
                                </div>
                            </template>
                            <template slot="cell-groups" slot-scope="{ row: user, value: groups }">
                                <div class="groups-index-field">
                                    <div v-for="group in (groups || [])" class="groups-index-field-item me-1 mb-1.5">{{ __(group.title) }}</div>
                                </div>
                            </template>
