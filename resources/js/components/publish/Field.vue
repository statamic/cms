    ...
    <div slot-scope="{ meta, value, loading: loadingMeta }" :class="classes">
        <div class="field-inner">
            <label class="publish-field-label" :class="{'font-bold': config.bold}" :for="fieldId">
                <span
                    :class="{ 'text-grey-600': syncable && isSynced }"
                    v-text="labelText"
                ...
                <span v-if="isReadOnly && !isSection" class="text-grey-500 font-normal text-2xs mx-1">
                    {{ isLocked ? __('Locked') : __('Read Only') }}
                </span>
                <svg-icon name="translate" class="h-4 ml-1 w-4 text-grey-600" v-if="isLocalizable && !isSection" v-tooltip.top="__('Localizable field')" />
                ...
                <button
                    ...
                    @click="$emit('desynced')"
                >
                    <svg-icon name="hyperlink" class="h-4 w-4 ml-1.5 mb-1 text-grey-600"
                ...
                <button
                    ...
                    @click="$emit('synced')"
                >
                    <svg-icon name="hyperlink-broken" class="h-4 w-4 ml-1.5 mb-1 text-grey-600"
