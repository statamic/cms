    ...
    <div slot-scope="{ meta, value, loading: loadingMeta }" :class="classes">
        <div class="field-inner">
            <label v-if="showLabel" class="publish-field-label" :class="{'font-bold': config.bold}" :for="fieldId">
                ...
                <avatar v-if="isLocked" :user="lockingUser" class="w-6 h-6 rounded-full -mt-px ms-2 me-2" v-tooltip="lockingUser.name" />
                <span v-if="isReadOnly && !isTab && !isSection" class="text-gray-500 font-normal text-2xs me-1">
                    {{ isLocked ? __('Locked') : __('Read Only') }}
                </span>
                <svg-icon name="translate" class="h-4 me-1 w-4 text-gray-600" v-if="isLocalizable && !isTab" v-tooltip.top="__('Localizable field')" />
