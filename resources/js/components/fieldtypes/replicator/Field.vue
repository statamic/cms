<template>
    ...
    <div class="p-4___REPLACED m-0" :class="classes">

        <label class="block">
            {{ display }}
            <i class="required" v-if="field.required">*</i>
            <span v-if="isReadOnly" class="text-grey-50 font-normal text-2xs mx-1___REPLACED" v-text="__('Read Only')" />
        </label>
        ...
        <div v-if="hasError">
            <small class="help-block text-red mt-2___REPLACED" v-for="(error, i) in errors" :key="i" v-text="error" />
        </div>
