    ...
    <set-picker v-else :enabled="enabled" :sets="groups" align="center" @added="addSet">
        <template #trigger>
            ...
            >
                ...
                />
                <button
                    class="absolute inset-0 h-full w-full opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 transition-opacity delay-10 duration-250 cursor-pointer"
                    tabindex="-1"
                >
                ...
                <Button v-if="enabled" round icon="plus" size="sm" class="-my-4 z-3 opacity-0 group-hover:opacity-100 focus:opacity-100 transition-opacity delay-10 focus:delay-0 duration-250" />
            </div>
        </template>
    </set-picker>
</template>
