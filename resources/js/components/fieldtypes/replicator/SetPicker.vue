            ...
            <div class="max-h-[21rem] overflow-auto p-1.5">
                ...
                >
                    ...
                    <div v-if="item.type === 'set'" @click="addSet(item.handle)" class="group flex items-center rounded-xl p-2.5 gap-2 sm:gap-3">
                        ...
                        <!-- When align is 'center' (e.g. in between sets), try to show hover card on right side.
                             Otherwise, let the component use its default positioning. -->
                        <ui-hover-card :delay="0" :open="selectionIndex === i" :side="align === 'center' ? 'right' : undefined">
                            <template #trigger>
                                <div class="flex-1">
                                    <div class="line-clamp-1 text-sm text-gray-900 dark:text-gray-200">
                                        {{ __(item.display || item.handle) }}
