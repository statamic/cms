<template>
    ...
    <div class="px-4 py-2 flex flex-wrap text-sm bg-gray-200 dark:bg-dark-700 border-t border-b dark:border-dark-900 shadow-inner">
        ...
        >
            <svg-icon name="micro/chevron-right" class="text-gray-700 h-4 w-4 me-1 rtl:rotate-180" v-if="index !== 0" />
            <svg-icon name="folder-home" class="me-2 h-5 w-5 text-blue-400 group-hover:text-blue-600" v-if="part === '/'" />
            <svg-icon name="folder" class="me-2 h-5 w-5 text-blue-400 group-hover:text-blue-600" v-else />
