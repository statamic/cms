<template>
    ...
    <div>
        ...
        <modal :show="show" class="modal-login" :shake="hasErrors">
            ...
            <template slot="body">
                <div class="mb-4">
                    <label :class="{ 'text-rose-500': errors.password.length }">{{ __('Password for :username', { username: this.username }) }} <i class="required">*</i></label>
                    <input type="password" name="password" class="input-text" v-model="password" ref="password" @keydown.enter.prevent="submit" />
                    <small class="block text-rose-500 mt-2" v-if="errors.username.length">{{ errors.username[0] }}</small>
                    <small class="block text-rose-500 mt-2" v-if="errors.password.length">{{ errors.password[0] }}</small>
                </div>
            </template>
