    ...
    <div>
        <popover v-if="isNotYetFavorited" ref="popper" placement="auto-end" :offset="[28, 10]">
            <template slot="trigger">
                <button @click="shown" slot="reference" class="h-6 w-6 block outline-none p-1 text-grey hover:text-grey-800" v-tooltip="__('Pin to Favorites')" :aria-label="__('Pin to Favorites')">
        ...
        <div v-else>
            <button @click="remove" class="h-6 w-6 block outline-none p-1 text-grey hover:text-grey-800" v-tooltip="__('Unpin from Favorites')" :aria-label="__('Unpin from Favorites')">
