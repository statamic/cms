...
<template>
    <component :is="tag" :class="badgeClasses" :href="props.href" data-ui-badge>
        <span v-if="props.prepend" class="me-0.5 border-e border-dashed pe-1.5 font-medium">{{ prepend }}</span>
        <Icon v-if="icon" :name="icon" />
        <slot v-if="hasDefaultSlot" />
        <template v-else>{{ text }}</template>
        <Icon v-if="iconAppend" :name="iconAppend" />
        <span v-if="props.append" class="ms-0.5 border-s border-dashed ps-1.5 font-medium">{{ append }}</span>
    </component>
</template>
