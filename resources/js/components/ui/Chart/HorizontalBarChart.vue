...
const iconStrokeClass = (index: number): string => `summary-bar-chart__icon-stroke--${Math.min(colorIndex(index), chartClasses.length - 1) + 1}`;
</script>
                ...
                >
                    ...
                    <span v-if="showMarker && markerPosition === 'before-label'" class="flex items-center gap-2">
                        <slot name="marker" :item="item" :index="index">
                            <Icon v-if="item.icon" :name="item.icon" :class="iconStrokeClass(index)" class="summary-bar-chart__icon-stroke size-3.5 shrink-0" />
