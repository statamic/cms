...
const columns = computed(() => props.showMarker
...
<template>
    <figure class="grid p-6" role="img" :aria-label="accessibleLabel" data-ui-horizontal-lollipop-chart>
        <ol :class="columns" class="grid items-center list-none m-0 gap-x-2.25 gap-y-2.5 p-0 pt-4" aria-hidden="true">
            <li v-for="(item, index) in items" :key="item.id ?? item.label ?? index" class="contents">
                ...
                <div class="flex items-center gap-1">
                    <div :style="{ width: `${item.value ?? item.percent}%` }" class="h-px bg-gray-200 dark:bg-gray-600" />
