...
abstract class UserGroupRepository implements RepositoryContract
{
    ...
    public function blueprint()
    {
        $blueprint = Blueprint::find('user_group') ?? Blueprint::makeFromFields([
            'title' => ['type' => 'text', 'display' => __('Title'), 'listable' => true, 'validate' => ['required'], 'instructions' => __('Usually a plural noun, like Editors or Photographers')],
            'handle' => ['type' => 'slug', 'display' => __('Handle'), 'listable' => true, 'validate' => ['required'], 'instructions' => __('Used to reference this user group on the frontend. It\'s non-trivial to change later.')],
