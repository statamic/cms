...
class EntriesController extends CpController
{
    ...
    public function edit(Request $request, $collection, $entry)
    {
        ...
        $viewData = [
            ...
            'titleFormat' => $collection->autoGeneratesTitles() && User::current()->can('update', $entry) ? [
                'url' => cp_route('collections.entries.title-format.edit', [$collection->handle(), $entry->id()]),
                'fields' => $entry->autoGeneratedTitleFields(),
            ] : null,
    ...
    public function create(Request $request, $collection, $site)
    {
        ...
        $entry = Entry::make()->collection($collection)->locale($site->handle());
        ...
        $viewData = [
            ...
            'titleFormat' => $collection->autoGeneratesTitles() ? [
                'url' => cp_route('collections.entries.title-format.create', [$collection->handle(), $site->handle()]),
                'fields' => $entry->autoGeneratedTitleFields(),
            ] : null,
            'values' => $values->all(),
            'extraValues' => [
    ...
    private function resolveSlug($request)
    {
        return function ($entry) use ($request) {
            // An auto generated slug lags behind the values it came from, so we derive it here instead.
            if ($request->slug && ! $request->boolean('_auto_slug')) {
                return $request->slug;
            }
            ...
            $title = $entry->collection()->autoGeneratesTitles() ? null : $request->title;

            return Str::slug($title ?? $entry->autoGeneratedTitle(), '-', $entry->site()->lang());
        };
    }
