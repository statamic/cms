...
class RelationshipFieldtypeController extends CpController
{
    ...
    {
        $config_encode = mb_convert_encoding(base64_decode($request->config), 'UTF-8', mb_list_encodings());
        $config = json_decode(!empty($config_encode) ? $config_encode : base64_decode($request->config), true);
