...
class TemplateTest extends ParserTestCase
{
    ...
    public function test_string_concat_casts_data_to_string()
    {
        $this->assertSame(10, intval($this->renderString('{{ 5 + 5 }}')));
        $this->assertSame('55', $this->renderString('{{ "5" + 5 }}'));
        $this->assertSame('55', $this->renderString('{{ 5 + "5" }}'));
        $this->assertSame('55', $this->renderString('{{ 5 &= 5 }}'));
    }

    public function test_multiple_conditions_in_ternary_are_properly_evaluated()
    {
        $template = "{{ response_code == 404 || hide_sidebar ? 'lg:w-3/4 xl:w-4/5' : 'xl:w-3/5' }}";

        $this->assertSame('xl:w-3/5', $this->renderString($template, []));
        $this->assertSame('xl:w-3/5', $this->renderString($template, ['response_code' => 200, 'hide_sidebar' => false]));
        $this->assertSame('lg:w-3/4 xl:w-4/5', $this->renderString($template, ['response_code' => 404, 'hide_sidebar' => false]));
        $this->assertSame('lg:w-3/4 xl:w-4/5', $this->renderString($template, ['response_code' => 404, 'hide_sidebar' => true]));
        $this->assertSame('lg:w-3/4 xl:w-4/5', $this->renderString($template, ['response_code' => 200, 'hide_sidebar' => true]));
    }

    public function test_interpolation_works_in_modifier_arguments()
    {
        $this->assertEquals('twodeep', $this->renderString('{{ default_key|ensure_right:{second_key} }}', $this->variables, true));
    }

    public function test_nested_interpolation_can_resolve_variables()
    {
        ...
        $this->assertEquals('Hello wilderness - two', $this->renderString("{{ test variable='{ true ? 'Hello wilderness - {default_key}' : 'fail' }' }}", $this->variables, true));
    }

    public function test_environment_replaces_interpolations_for_dynamic_string_comparisons()
    {
        $this->assertEquals('yes', $this->renderString("{{ if 'test-wilderness' == 'test-{associative[default_key]}' }}yes{{ else }}nope{{ /if }}", $this->variables));
        $this->assertEquals('yes', $this->renderString("{{ if 'test-{'wilderness'}' == 'test-{associative[default_key]}' }}yes{{ else }}nope{{ /if }}", $this->variables));
        $this->assertEquals('nope', $this->renderString("{{ if 'test-wilderness' == 'test-not-{associative[default_key]}' }}yes{{ else }}nope{{ /if }}", $this->variables));
    }

    public function test_nested_interpolation_can_resolve_variables_with_mixed_braces()
    {
        (new class extends Tags
        {
            public static $handle = 'test';
    ...
    public function test_interpolation_works_with_single_braces()
    {
        (new class extends Tags
        {
            public static $handle = 'test';
        ...
        $this->assertEquals('Hello wilderness', $this->renderString("{{ test variable='{ true ? 'Hello wilderness' : 'fail' }' }}", [], true));
        $this->assertEquals('fail', $this->renderString("{{ test variable='{ false ? 'Hello wilderness' : 'fail' }' }}", [], true));
    }

    public function test_interpolation_works_with_double_braces()
    {
        (new class extends Tags
        {
            public static $handle = 'test';
    ...
    public function test_interpolation_can_pull_variables_in_scope()
    {
        (new class extends Tags
        {
            public static $handle = 'test';
        ...
        $this->assertEquals('two', $this->renderString("{{ test variable='{ true ? default_key : first_key }' }}", $this->variables, true));
        $this->assertEquals('three', $this->renderString("{{ test variable='{ false ? default_key : first_key }' }}", $this->variables, true));
    }

    public function test_double_brace_interpolation_can_pull_variables_in_scope()
    {
        (new class extends Tags
        {
            public static $handle = 'test';
    ...
    public function test_interpolation_can_pull_path_variables_in_scope()
    {
        (new class extends Tags
        {
            public static $handle = 'test';
        ...
        $this->assertEquals('hello', $this->renderString("{{ test variable='{ true ? associative:one : first_key }' }}", $this->variables, true));
        $this->assertEquals('wilderness', $this->renderString("{{ test variable='{ false ? default_key : associative:two }' }}", $this->variables, true));
    }

    public function test_it_uses_tags_with_single_part_in_conditions()
    {
        (new class extends Tags
        {
    ...
    public function test_it_uses_tags_with_multiple_parts_in_conditions()
    {
        (new class extends Tags
        {
            public static $handle = 'truthy';
    ...
    public function test_recursive_hildren()
    {
        ...
        $this->assertEquals(StringUtilities::normalizeLineEndings($expected), $this->renderString($template, ['foo' => 'Bar'], true));
    }

    public function test_recursive_children_with_scope()
    {
        // the variables are inside RecursiveChildren@index
        $this->app['statamic.tags']['recursive_children'] = RecursiveChildren::class;
    ...
    public function test_it_aliases_callback_tag_pair_loop_using_the_as_param()
    {
        (new class extends Tags
        {
...
EOT;
        ...
        $this->assertEquals(StringUtilities::normalizeLineEndings($expected), $this->renderString($template, [], true));
    }

    public function test_it_aliases_array_tag_pairs_using_the_as_modifier()
    {
        $template = <<<'EOT'
{{ array as="stuff" }}
before
...
EOT;
        ...
        $this->assertEquals(StringUtilities::normalizeLineEndings($expected), $this->renderString($template, [
            'array' => [
                ['foo' => 'bar'],
                ['foo' => 'baz'],
                ['foo' => 'qux'],
    ...
    public function test_callback_tags_that_return_unparsed_simple_arrays_get_parsed_with_scope()
    {
        (new class extends Tags
        {
            public static $handle = 'tag';
...
EOT;
    ...
    public function test_scope_modifier_can_add_scopes()
    {
        $context = [
            'drink' => 'whisky',
            'food' => 'burger',
...
EOT;
        $this->assertEquals(StringUtilities::normalizeLineEndings($expected), $this->renderString($template, $context, true));
    }

    public function test_it_can_create_scopes()
    {
        $context = [
            'drink' => 'whisky',
            'food' => 'burger',
...
EOT;
        ...
        $this->assertEquals(StringUtilities::normalizeLineEndings($expected), trim($this->renderString($template, $context, true)));
    }

    public function test_it_can_reach_into_the_cascade()
    {
...
EOT;
        ...
        $results = (string) $this->parser($context)->cascade($cascade)->parse($template, $context);
        $this->assertEquals(StringUtilities::normalizeLineEndings($expected), StringUtilities::normalizeLineEndings($results));
    }

    public function test_callback_tag_pair_variables_get_context_merged_in_but_nulls_remain_null()
    {
        (new class extends Tags
        {
            public static $handle = 'tag';
...
EOT;
    ...
    public function test_it_parses_php_when_enabled()
    {
        $this->assertEquals(
            'Hello wilderness!',
    ...
    {
        $this->assertEquals('hello', (string) $this->parser($this->variables)->allowPhp(true)->parse('{{ associative }}<?php echo $one; ?>{{ /associative }}', $this->variables));
    }

    public function test_empty_values_are_not_overridden_by_previous_iteration_with_parsing()
    {
    ...
    public function test_it_counts_query_builder_results_in_conditions()
    {
    ...
    public function test_it_does_not_accept_multidimensional_array()
    {
    ...
    public function test_it_does_not_accept_sequences()
    {
        $this->expectException(\InvalidArgumentException::class);
        $this->expectExceptionMessage('Expecting an associative array');
    ...
    public function test_it_throws_exception_for_non_arrayable_data_object()
    {
        try {
    ...
    public function test_it_throws_exception_for_unsupported_data_value()
    {
        try {
    ...
    public function test_it_cannot_cast_non_arrayable_objects_to_arrays_when_using_tag_pairs()
    {
    ...
    public function test_it_doesnt_output_anything_if_object_cannot_be_cast_to_a_string()
    {
        Log::shouldReceive('debug')->once()
    ...
    public function test_non_arrays_cannot_be_looped()
    {
        Log::shouldReceive('debug')->once()
    ...
    public function test_unclosed_array_variable_pairs_should_be_bull()
    {
        Log::shouldReceive('debug')->once()
    ...
    public function test_string_variable()
    {
        $template = '{{ string }}';
    ...
    public function test_string_variables_with_tight_braces()
    {
        $template = '{{string}}';
    ...
    public function test_array_variables()
    {
        $template = <<<'EOT'
before
{{ simple }}
...
EOT;
        ...
        $this->assertEquals(StringUtilities::normalizeLineEndings($expected), $this->renderString($template, $this->variables));
    ...
    public function test_complex_array_variable()
    {
        $template = <<<'EOT'
before
{{ complex }}
...
EOT;
    ...
    public function test_variables_starting_with_if_arent_treated_as_if_statements()
    {
    ...
    public function test_static_strings_with_double_quotes_can_be_modified()
    {
    ...
    public function test_static_strings_with_single_quotes_can_be_modified()
    {
        $template = "{{ 'Thundercats are Go!' | upper }}";
    ...
    public function test_modified_non_existent_variables_should_be_null()
    {
    ...
    public function test_objects_are_considered_truthy()
    {
    ...
    public function test_conditions_on_overlapping_variable_names()
    {
    ...
    public function test_loop_with_param_inside_condition_matching_variable_name()
    {
    ...
    public function test_ternary_condition()
    {
    ...
    public function test_ternary_condition_with_dynamic_array()
    {
    ...
    public function test_ternary_condition_isnt_too_greedy()
    {
    ...
    public function test_ternary_condition_with_modifiers()
    {
    ...
    public function test_ternary_condition_with_modifiers_and_dynamic_array()
    {
    ...
    public function test_truth_coalescing_inside_loop()
    {
    ...
    public function test_single_standard_string_modifier_tight()
    {
    ...
    public function test_chained_standard_string_modifiers_tight()
    {
    ...
    public function test_single_standard_string_modifier_relaxed()
    {
        $template = '{{ string | upper }}';
    ...
    public function test_chained_standard_string_modifiers_relaxed()
    {
        $template = '{{ string | upper | lower }}';
    ...
    public function test_chained_standard_string_modifiers_from_dynamic_array_relaxed()
    {
    ...
    public function test_single_parameter_string_modifier()
    {
    ...
    public function test_single_parameter_string_from_array_modifier()
    {
        $this->assertEquals(
            'WILDERNESS',
    ...
    public function test_chained_parameter_string_modifiers()
    {
    ...
    public function test_single_standard_array_modifier_relaxed()
    {
    ...
    public function test_single_standard_array_modifier_tight()
    {
        $template = '{{ simple|length }}';
    ...
    public function test_chained_standard_array_modifiers_tight_on_content()
    {
    ...
    public function test_chained_standard_modifiers_relaxed_on_content()
    {
        $template = '{{ content | markdown | lower }}';
    ...
    public function test_chained_parameter_modifiers_on_content()
    {
    ...
    public function test_conditions_with_modifiers()
    {
    ...
    public function test_conditions_with_relaxed_modifiers()
    {
        $template = "{{ if string | upper == 'HELLO WILDERNESS' }}yes{{ endif }}";
    ...
    public function test_array_modifiers_gets_parsed()
    {
    ...
    public function test_array_modifiers_on_collections_get_parsed()
    {
        $template = '{{ simple limit="1" }}{{ value }}{{ /simple }}';
    ...
    public function test_scope_glue()
    {
    ...
    public function test_empty_values_are_not_overridden_by_previous_iteration()
    {
        $variables = [
            'loop' => [
                [
    ...
    public function test_nested_array_syntax()
    {
        $variables = [
            'hello' => [
                'world' => [
    ...
    public function test_it_doesnt_parse_noparse_tags()
    {
    ...
    public function test_it_doesnt_parse_data_in_noparse_modifiers()
    {
        $variables = [
            'string' => 'hello',
    ...
    public function test_it_doesnt_parse_data_in_noparse_modifiers_with_null_coalescence()
    {
        $variables = [
            'string' => 'hello',
            'content' => 'before {{ string }} after',
    ...
    public function test_it_doesnt_parse_tags_prefixed_with_an_at_symbol()
    {
        $this->assertEquals('foo {{ bar }} baz', $this->renderString('foo @{{ bar }} baz'));
    }

    public function test_it_doesnt_parse_tags_prefixed_with_an_at_symbol_over_multiple_lines()
    {
        $template = <<<'EOT'
@{{ foo }}
bar
...
EOT;
        ...
        $this->assertEquals(StringUtilities::normalizeLineEndings($expected), $this->renderString($template, ['baz' => 'BAZ']));
    }

    public function test_it_doesnt_parse_tags_prefixed_with_an_at_symbol_over_tags_in_multiple_lines()
    {
        $template = <<<'EOT'
@{{ foo }} {{ qux }}
bar
...
EOT;
        ...
        $this->assertEquals(StringUtilities::normalizeLineEndings($expected), $this->renderString($template, ['baz' => 'BAZ', 'qux' => 'QUX']));
    }

    public function test_it_doesnt_parse_multiline_tags_prefixed_with_an_at_symbol_over_tags_in_multiple_lines()
    {
        $template = <<<'EOT'
@{{ foo
  bar:baz="qux"
...
EOT;
    ...
    public function test_it_doesnt_parse_tags_prefixed_with_an_at_symbol_containing_nested_tags()
    {
        $this->assertEquals('{{ foo bar="{baz}" }}', $this->renderString('@{{ foo bar="{baz}" }}'));
    }

    public function test_it_accepts_an_arrayable_object()
    {
        $this->assertEquals(
            'Hello World',
    ...
    public function test_it_gets_augmented_value()
    {
        $fieldtype = new class extends Fieldtype
        {
    ...
    public function test_it_expands_augmented_value_when_used_as_an_array()
    {
        $fieldtype = new class extends Fieldtype
        {
    ...
    public function test_it_gets_nested_values_from_augmentable_objects()
    {
    ...
    public function test_it_loops_over_value_object()
    {
        $fieldtype = new class extends Fieldtype
        {
            public function augment($values)
    ...
    public function test_it_gets_nested_values_from_value_objects()
    {
    ...
    public function test_it_gets_nested_values_from_nested_value_objects()
    {
        $value = new Value(['foo' => 'bar'], 'test');
    ...
    public function test_it_gets_nested_values_from_within_nested_value_objects()
    {
        $value = new Value([
            'foo' => ['nested' => 'bar'],
        ], 'test');
    ...
    public function test_it_casts_objects_to_string_when_using_single_tags()
    {
        $object = new class
        {
    ...
    public function test_it_automatically_augments_augmentable_objects_when_using_tag_pairs()
    {
    ...
    public function test_it_casts_arrayable_objects_to_arrays_when_using_tag_pairs()
    {
    ...
    public function test_static_strings_with_double_quotes_should_be_left_alone()
    {
    ...
    public function test_static_strings_with_single_quotes_should_be_left_alone()
    {
        $template = "{{ 'Thundercats are Go!' }}";
    ...
    public function test_non_existent_variables_should_be_null()
    {
        $template = '{{ missing }}';
    ...
    public function test_accessing_strings_as_arrays_returns_null()
    {
        $this->assertEquals('bar, ><', $this->renderString('{{ foo }}, >{{ foo:test }}<', ['foo' => 'bar']));
    }

    public function test_single_condition()
    {
    ...
    public function test_multiple_or_conditions()
    {
     ...
     **/
    public function test_it_parses_single_and_tag_pairs_with_modifiers()
    {
    ...
    public function test_multiple_and_conditions()
    {
    ...
    public function test_it_can_access_ArrayableString_properties_by_colon_notation()
    {
        $fieldtype = new class extends Fieldtype
        {
            public function augment($value)
    ...
    public function test_variable_tag_pair_get_context_merged_in_except_for_nulls()
    {
        $context = [
            'drink' => 'whisky',
            'food' => 'burger',
...
juice burger --
EOT;
        $this->assertEquals(StringUtilities::normalizeLineEndings($expected), $this->renderString($template, $context));
    }

    public function test_it_can_treat_a_ArrayableString_object_as_an_array()
    {
        $fieldtype = new class extends Fieldtype
        {
            public function augment($value)
    ...
    public function test_it_automatically_augments_collections_when_using_tag_pairs()
    {
        $augmentable = collect([
    ...
    public function test_modifiers_on_tag_pairs_receive_the_augmented_value()
    {
        $fieldtype = new class extends Fieldtype
        {
            public function augment($value)
    ...
    public function test_or_existence_conditions()
    {
    ...
    public function test_single_braces_should_not_be_parsed()
    {
        $template = '{string}';
    ...
    public function test_when_a_loop_is_a_value_object_with_an_empty_array_it_get_parsed_as_one()
    {
        $template = <<<'EOT'
before
{{ simple }}
...
EOT;
    ...
    public function test_ternary_condition_with_multiple_lines()
    {
        $template = <<<'EOT'
{{ string
    ? "Pass"
...
EOT;
    ...
    public function test_dated_condition_with_chained_relaxed_modifiers_with_spaces_in_arguments()
    {
    ...
    public function test_ternary_escapes_quotes_properly()
    {
    ...
    public function test_associative_array_variable()
    {
        $template = <<<'EOT'
before
{{ associative }}
...
EOT;
    ...
    public function test_it_does_stuff_in_issue_2456()
    {
    ...
    public function test_it_does_stuff_in_issue_2537()
    {
    ...
    public function test_it_compares_to_a_string_that_looks_like_array_access()
    {
    ...
    public function test_empty_view_error_bags_are_considered_empty_in_conditions()
    {
    ...
    public function test_it_can_remove_escaping_characters_from_tenary_output_with_truth_coalescence()
    {
        $vars = [
            'truthy' => true,
    ...
    public function test_it_automatically_augments_augmentable_objects_when_looping_with_modifier()
    {
        $loop = [
            new AugmentableObject(['one' => 'foo', 'two' => 'bar']),
            new AugmentableObject(['one' => 'baz', 'two' => 'qux']),
    ...
    public function test_empty_collections_are_considered_empty_in_conditions()
    {
    ...
    public function test_it_applies_modifier_on_different_array_syntax()
    {
        $vars = [
            'key' => 'entries',
            'source' => [
    ...
    public function test_it_outputs_the_value_when_a_ArrayableString_object_is_used_as_string()
    {
        $fieldtype = new class extends Fieldtype
        {
            public function augment($value)
    ...
    public function test_parameter_style_modifier_with_colon_prefix_will_get_the_values_from_context()
    {
    ...
    public function test_value_objects_with_antlers_disabled_do_not_get_parsed()
    {
        $fieldtype = new class extends Fieldtype
        {
            public function augment($value)
    ...
    public function test_it_can_use_ArrayableString_objects_in_conditions()
    {
        $fieldtype = new class extends Fieldtype
        {
            public function augment($value)
    ...
    public function test_callback_tags_that_return_unparsed_multidimensional_arrays_get_parsed()
    {
        (new class extends Tags
        {
            public static $handle = 'tag';
...
EOT;
        ...
        $this->assertEquals(StringUtilities::normalizeLineEndings($expected), $this->renderString($template, ['string' => 'Hello wilderness'], true));
    }

    public function test_callback_tags_that_return_empty_arrays_get_parsed_with_no_results()
    {
        (new class extends Tags
        {
            public static $handle = 'tag';
...
EOT;
        ...
        $this->assertEquals(StringUtilities::normalizeLineEndings($expected), $this->renderString($template, $this->variables, true));
    }

    public function test_callback_tags_that_return_collections_get_parsed()
    {
        (new class extends Tags
        {
            public static $handle = 'tag';
...
EOT;
    ...
    public function test_callback_tags_that_return_value_objects_gets_parsed()
    {
        (new class extends Tags
        {
            public static $handle = 'tag';
    ...
    public function test_callback_tags_that_return_value_objects_with_antlers_gets_parsed()
    {
        (new class extends Tags
        {
            public static $handle = 'tag';
    ...
    public function test_callback_tags_that_return_value_objects_with_antlers_disabled_does_not_get_parsed()
    {
        (new class extends Tags
        {
            public static $handle = 'tag';
    ...
    public function test_callback_tags_that_return_unparsed_simple_arrays_get_parsed()
    {
        (new class extends Tags
        {
            public static $handle = 'tag';
...
EOT;
    ...
    public function test_value_objects_with_antlers_gets_parsed()
    {
        $fieldtype = new class extends Fieldtype
        {
            public function augment($value)
    ...
    public function test_it_automatically_augments_augmentable_objects_when_returned_from_a_callback_tag()
    {
        (new class extends Tags
        {
            public static $handle = 'tag';
    ...
    public function test_it_parses_value_objects_values_when_configured_to_do_so()
    {
...
EOT;
        ...
        $this->assertEquals(StringUtilities::normalizeLineEndings($expected), (string) $this->renderString($template, $variables));
    ...
    public function test_accessing_string_as_array_which_exists_as_callback_calls_the_callback()
    {
        (new class extends Tags
        {
    ...
    public function test_it_doesnt_parse_noparse_tags_inside_callbacks()
    {
        (new class extends Tags
        {
            public static $handle = 'tag';
...
EOT;
        ...
        $this->assertSame(StringUtilities::normalizeLineEndings($expected), $this->renderString($template, $this->variables, true));
    }

    public function test_it_doesnt_parse_data_in_noparse_modifiers_inside_callbacks()
    {
        (new class extends Tags
        {
            public static $handle = 'tag';
...
EOT;
        ...
        $this->assertEquals(StringUtilities::normalizeLineEndings($expected), $parsed);
    }
}
